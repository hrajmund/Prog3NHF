    ...
    public void mouseClicked(MouseEvent e) {
        if(e.getButton() == MouseEvent.BUTTON3 && e.getClickCount() == 2){
            ...
            for (int i = 0; i < 9; i++) {
                for (int j = 0; j < 9; j++) {
                    if (block.getRow() == i && block.getColumn() == j) {
                        BoardBlock srcPlayer1 = gameManager.board[gameManager.getPlayer(1).getX()][gameManager.getPlayer(1).getY()];
                        BoardBlock srcPlayer2 = gameManager.board[gameManager.getPlayer(2).getX()][gameManager.getPlayer(2).getY()];
                        if(gameManager.pathExists(gameManager.getP1()) || gameManager.pathExists(gameManager.getP2())){
                            if (j > 0) {
                            gameManager.board[i][j + 1].setState("B");
